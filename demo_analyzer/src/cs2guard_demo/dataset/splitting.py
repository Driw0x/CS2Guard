...
def create_dataset_splits(dataset: pd.DataFrame, train_ratio: float=0.7, validation_ratio: float=0.15, test_ratio: float=0.15, random_state: int=42) -> pd.DataFrame:
    ...
    targets = {'train': total_samples * train_ratio, 'validation': total_samples * validation_ratio, 'test': total_samples * test_ratio}
    current_sizes = {'train': 0, 'validation': 0, 'test': 0}
    group_assignments = {}

    for group in groups:
        enabled_splits = [split for split, target in targets.items() if target > 0.0]
        selected_split = min(enabled_splits, key=lambda split: current_sizes[split] / targets[split])
