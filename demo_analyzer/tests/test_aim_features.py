...
def test_angular_velocity():
    ...
    assert math.isclose(yaw_velocity, 20.0, abs_tol=1e-6)
    assert math.isclose(pitch_velocity, -8.0, abs_tol=1e-6)
...
def test_angular_velocity_wraparound():
    ...
    assert math.isclose(yaw_velocity, 20.0, abs_tol=1e-6)
    assert math.isclose(pitch_velocity, 0.0, abs_tol=1e-6)
...
def test_angular_velocity_negative():
    ...
    assert math.isclose(yaw_velocity, -20.0, abs_tol=1e-6)
    assert math.isclose(pitch_velocity, -10.0, abs_tol=1e-6)
...
def test_angular_speed():
    speed = angular_speed(
        yaw_velocity=300.0,
        pitch_velocity=400.0,
    )

    assert math.isclose(speed, 500.0, abs_tol=1e-6)


def test_angular_acceleration():
    yaw_acceleration, pitch_acceleration = angular_acceleration(
        previous_yaw_velocity=100.0,
        previous_pitch_velocity=50.0,
        current_yaw_velocity=150.0,
        current_pitch_velocity=30.0,
        delta_time=0.5,
    )

    assert math.isclose(yaw_acceleration, 100.0, abs_tol=1e-6)
    assert math.isclose(pitch_acceleration, -40.0, abs_tol=1e-6)


def test_angular_acceleration_negative():
    yaw_acceleration, pitch_acceleration = angular_acceleration(
        previous_yaw_velocity=200.0,
        previous_pitch_velocity=100.0,
        current_yaw_velocity=100.0,
        current_pitch_velocity=50.0,
        delta_time=0.5,
    )

    assert math.isclose(yaw_acceleration, -200.0, abs_tol=1e-6)
    assert math.isclose(pitch_acceleration, -100.0, abs_tol=1e-6)
...
def test_angular_acceleration_invalid_delta_time():
    ...
    with pytest.raises(ValueError):
        angular_acceleration(
            previous_yaw_velocity=0.0,
            previous_pitch_velocity=0.0,
            current_yaw_velocity=100.0,
            current_pitch_velocity=100.0,
            delta_time=0.0,
        )
...
def test_angular_acceleration_magnitude():
    magnitude = angular_acceleration_magnitude(
        yaw_acceleration=300.0,
        pitch_acceleration=400.0,
    )

    assert math.isclose(magnitude, 500.0, abs_tol=1e-6)
...
def test_target_acquisition_requires_stability():
    ...
    acquisition = find_target_acquisition(
        distances,
        threshold=3.0,
...
def test_reaction_time():
    value = reaction_time(
        stimulus_index=100,
        response_index=106,
        sample_interval=1 / 64,
    )

    assert math.isclose(value, 0.09375, abs_tol=1e-6)
...
def test_mean_tracking_error():
    ...
    assert math.isclose(result, 2.5, abs_tol=1e-6)
...
def test_max_tracking_error():
    ...
    assert math.isclose(result, 4.5, abs_tol=1e-6)
...
def test_tracking_time_on_target_ratio():
    ...
    result = tracking_time_on_target_ratio(
        distances,
        threshold=3.0,
    )

    assert math.isclose(result, 0.75, abs_tol=1e-6)
...
def test_find_potential_aim_snaps_detects_snap():
    ...
    timestamps = [0.00, 0.03, 0.06, 0.09]
...
def test_find_potential_aim_snaps_duration_too_long():
    distances = [20.0, 14.0, 8.0, 3.0]
    speeds = [0.0, 250.0, 400.0, 300.0]
    timestamps = [0.00, 0.10, 0.20, 0.30]
...
def test_find_potential_aim_snaps_ignores_small_correction():
    ...
    timestamps = [0.00, 0.03, 0.06]

    assert find_potential_aim_snaps(
        distances,
        speeds,
        timestamps,
        start_distance_threshold=10.0,
    ) == []
...
def test_associate_shot_inside_sequence():
    sequences = associate_shots_with_aim_sequences(
        sequence_windows=[(1, 3)],
        sample_ticks=[100, 101, 102, 103, 104],
        shot_ticks=[102],
    )
...
def test_associate_no_sequences():
    assert associate_shots_with_aim_sequences(
        sequence_windows=[],
