...
class FakeParser:
    ...
    def parse_ticks(self, fields):
        return pd.DataFrame({'tick': [50, 100, 101, 102], 'steamid': [1, 1, 2, 2], 'name': ['PlayerA', 'PlayerA', 'PlayerB', 'PlayerB'], 'team_name': ['CT', 'CT', 'TERRORIST', None], 'X': [10.0, 20.0, 30.0, None], 'Y': [15.0, 25.0, 35.0, None], 'Z': [5.0, 6.0, 7.0, None], 'yaw': [90.0, 100.0, 110.0, None], 'pitch': [5.0, 10.0, 15.0, None]})

    def parse_event(self, event_name):
        if event_name == 'round_start':
            return pd.DataFrame({'round': [1, 1, 2, 3], 'tick': [0, 99, 200, 300]})

        if event_name == 'round_end':
            return pd.DataFrame({'reason': ['t_killed', 'ct_killed', 'bomb_exploded'], 'round': [1, 2, 3], 'tick': [80, 180, 280], 'winner': ['CT', 'T', 'T']})

        if event_name == 'weapon_fire':
            return pd.DataFrame({'tick': [50, 120], 'user_steamid': [1, 1], 'user_name': ['PlayerA', 'PlayerA'], 'weapon': ['ak47', 'ak47'], 'silenced': [False, False]})

        if event_name == 'player_hurt':
            return pd.DataFrame({'tick': [130], 'attacker_steamid': [1], 'attacker_name': ['PlayerA'], 'user_steamid': [2], 'user_name': ['PlayerB'], 'weapon': ['ak47'], 'dmg_health': [40], 'dmg_armor': [5], 'health': [60], 'armor': [95], 'hitgroup': ['chest']})

        if event_name == 'player_death':
            return pd.DataFrame({'tick': [150], 'attacker_steamid': [1], 'attacker_name': ['PlayerA'], 'user_steamid': [2], 'user_name': ['PlayerB'], 'weapon': ['ak47'], 'headshot': [True], 'distance': [20.0], 'hitgroup': ['head'], 'noscope': [False], 'penetrated': [0], 'thrusmoke': [False], 'attackerblind': [False], 'attackerinair': [False]})
...
def test_get_events():
    ...
    assert len(events) == 3
    assert [event.event_type for event in events] == ['shot', 'hit', 'kill']
    assert [event.tick for event in events] == [120, 130, 150]
