...
class FakeDemoParser:
    ...
    def get_match_metadata(self):
        return {'map_name': 'de_test', 'patch_version': 'test_patch', 'demo_version': 'test_demo'}

    def get_shots(self):
        return pd.DataFrame([{'tick': 40, 'user_steamid': 1, 'user_name': 'PlayerOne', 'weapon': 'weapon_ak47'}, {'tick': 50, 'user_steamid': 2, 'user_name': 'PlayerTwo', 'weapon': 'weapon_m4a1'}, {'tick': 55, 'user_steamid': 1, 'user_name': 'PlayerOne', 'weapon': 'weapon_flashbang'}, {'tick': 56, 'user_steamid': 2, 'user_name': 'PlayerTwo', 'weapon': 'weapon_knife'}])

    def get_hits(self):
        return pd.DataFrame([{'tick': 40, 'attacker_steamid': 1, 'attacker_name': 'PlayerOne', 'user_steamid': 2, 'user_name': 'PlayerTwo', 'weapon': 'weapon_ak47', 'dmg_health': 30, 'dmg_armor': 5, 'health': 70, 'armor': 95, 'hitgroup': 'chest'}, {'tick': 50, 'attacker_steamid': 2, 'attacker_name': 'PlayerTwo', 'user_steamid': 1, 'user_name': 'PlayerOne', 'weapon': 'weapon_m4a1', 'dmg_health': 40, 'dmg_armor': 0, 'health': 60, 'armor': 0, 'hitgroup': 'head'}, {'tick': 45, 'attacker_steamid': 1, 'attacker_name': 'PlayerOne', 'user_steamid': 1, 'user_name': 'PlayerOne', 'weapon': '', 'dmg_health': 1, 'dmg_armor': 0, 'health': 0, 'armor': 0, 'hitgroup': 'generic'}])

    def get_kills(self):
        return pd.DataFrame([{'tick': 40, 'attacker_steamid': 1, 'attacker_name': 'PlayerOne', 'user_steamid': 2, 'user_name': 'PlayerTwo', 'weapon': 'weapon_ak47', 'headshot': False, 'distance': 500.0, 'hitgroup': 'chest', 'noscope': False, 'penetrated': 0, 'thrusmoke': False}, {'tick': 50, 'attacker_steamid': 2, 'attacker_name': 'PlayerTwo', 'user_steamid': 1, 'user_name': 'PlayerOne', 'weapon': 'weapon_m4a1', 'headshot': True, 'distance': 250.0, 'hitgroup': 'head', 'noscope': False, 'penetrated': 1, 'thrusmoke': False}])
    ...
    def get_player_ticks_df(self):
        ...
        for steamid, name in [(1, 'PlayerOne'), (2, 'PlayerTwo')]:
            for tick in range(1, 65):
                rows.append({'tick': tick, 'steamid': steamid, 'name': name, 'team_name': 'TERRORIST' if steamid == 1 else 'CT', 'X': float(tick), 'Y': float(tick * 2), 'Z': 64.0, 'yaw': float(tick), 'pitch': float(tick) * 0.25})
...
def test_event_level_samples(monkeypatch, tmp_path):
    ...
    self_hits = dataset[(dataset['event_type'] == 'hit') & (dataset['steamid'] == dataset['victim_steamid'])]
...
def test_aim_features(monkeypatch, tmp_path):
    ...
    feature_columns = ['mean_angular_speed', 'max_angular_speed', 'std_angular_speed', 'mean_angular_acceleration', 'max_angular_acceleration', 'std_angular_acceleration']
...
def test_fit_normalization_stats():
    dataset = pd.DataFrame({'mean_angular_speed': [10.0, 20.0, 30.0], 'max_angular_speed': [20.0, 40.0, 60.0], 'std_angular_speed': [1.0, 2.0, 3.0], 'mean_angular_acceleration': [100.0, 200.0, 300.0], 'max_angular_acceleration': [200.0, 400.0, 600.0], 'std_angular_acceleration': [10.0, 20.0, 30.0]})
...
def test_normalize_features():
    dataset = pd.DataFrame({'window_id': ['a', 'b', 'c'], 'mean_angular_speed': [10.0, 20.0, 30.0], 'max_angular_speed': [20.0, 40.0, 60.0], 'std_angular_speed': [1.0, 2.0, 3.0], 'mean_angular_acceleration': [100.0, 200.0, 300.0], 'max_angular_acceleration': [200.0, 400.0, 600.0], 'std_angular_acceleration': [10.0, 20.0, 30.0]})
    stats = fit_normalization_stats(dataset, AIM_FEATURE_COLUMNS)
...
def test_apply_match_labels():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_a', 'match_b'], 'steamid': [1, 2, 3]})
    labels = pd.DataFrame({'match_id': ['match_a', 'match_b'], 'steamid': [pd.NA, pd.NA], 'label': ['legitimate', 'suspicious']})
    labeled = apply_labels(dataset, labels)
    assert labeled['label'].tolist() == ['legitimate', 'legitimate', 'suspicious']


def test_player_label_overrides_match_label():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_a'], 'steamid': [1, 2]})
    labels = pd.DataFrame({'match_id': ['match_a', 'match_a'], 'steamid': [pd.NA, 2], 'label': ['legitimate', 'suspicious']})
    labeled = apply_labels(dataset, labels)
...
def test_unlabeled_samples_remain_unlabeled():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_b'], 'steamid': [1, 2]})
    labels = pd.DataFrame({'match_id': ['match_a'], 'steamid': [pd.NA], 'label': ['legitimate']})
    labeled = apply_labels(dataset, labels)
...
def test_invalid_label_is_rejected():
    labels = pd.DataFrame({'match_id': ['match_a'], 'steamid': [pd.NA], 'label': ['cheater']})
...
def test_duplicate_player_labels_are_rejected():
    labels = pd.DataFrame({'match_id': ['match_a', 'match_a'], 'steamid': [1, 1], 'label': ['legitimate', 'suspicious']})
...
def test_leakage_groups_connect_shared_players():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_a', 'match_b', 'match_b', 'match_c'], 'steamid': [1, 2, 2, 3, 4]})
...
def test_create_dataset_splits():
    ...
    for match_index in range(10):
        ...
        for sample_index in range(10):
            rows.append({'match_id': match_id, 'steamid': steamid, 'sample': sample_index})

    dataset = pd.DataFrame(rows)
    split_dataset = create_dataset_splits(dataset, train_ratio=0.7, validation_ratio=0.15, test_ratio=0.15, random_state=42)
...
def test_match_never_crosses_splits():
    ...
    for match_index in range(6):
        for sample_index in range(5):
            rows.append({'match_id': f'match_{match_index}', 'steamid': 1000 + match_index, 'sample': sample_index})
...
def test_player_never_crosses_splits():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_a', 'match_b', 'match_b', 'match_c', 'match_d', 'match_e'], 'steamid': [1, 2, 2, 3, 4, 5, 6]})
    split_dataset = create_dataset_splits(dataset)
    validate_no_data_leakage(split_dataset)
...
def test_split_is_reproducible():
    dataset = pd.DataFrame({'match_id': [f'match_{index}' for index in range(20)], 'steamid': [1000 + index for index in range(20)]})
...
def test_invalid_split_ratios_are_rejected():
    dataset = pd.DataFrame({'match_id': ['match_a'], 'steamid': [1]})
...
def test_leakage_validation_detects_match_leakage():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_a'], 'steamid': [1, 2], 'split': ['train', 'test']})
...
def test_leakage_validation_detects_player_leakage():
    dataset = pd.DataFrame({'match_id': ['match_a', 'match_b'], 'steamid': [1, 1], 'split': ['train', 'validation']})
...
def test_generate_dataset_statistics():
    event_dataset = pd.DataFrame({'match_id': ['match_a', 'match_a', 'match_b'], 'steamid': [1, 2, 3], 'event_type': ['shot', 'hit', 'kill']})
    player_dataset = pd.DataFrame({'match_id': ['match_a', 'match_a', 'match_b'], 'steamid': [1, 2, 3]})
    temporal_windows = pd.DataFrame({'window_id': ['window_a', 'window_a', 'window_b', 'window_b'], 'match_id': ['match_a', 'match_a', 'match_b', 'match_b']})
    aim_features = pd.DataFrame({'window_id': ['window_a', 'window_b'], 'match_id': ['match_a', 'match_b'], 'steamid': [1, 3], 'mean_angular_speed': [10.0, 20.0], 'max_angular_speed': [20.0, 40.0], 'std_angular_speed': [2.0, 4.0], 'mean_angular_acceleration': [100.0, 200.0], 'max_angular_acceleration': [200.0, 400.0], 'std_angular_acceleration': [20.0, 40.0]})
...
def test_statistics_include_labels_and_splits():
    empty = pd.DataFrame()
    aim_features = pd.DataFrame({'window_id': ['a', 'b', 'c'], 'match_id': ['m1', 'm2', 'm3'], 'steamid': [1, 2, 3], 'label': ['legitimate', 'legitimate', 'suspicious'], 'split': ['train', 'validation', 'test']})
